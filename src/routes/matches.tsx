...
function MatchesPage() {
  ...
  const { data: myPicks = {} } = useQuery({
    ...
    queryFn: async () => {
      ...
      data?.forEach((p) => { map[p.match_id] = p.picked as any; });
      return map;
    },
  });
  ...
  if (!player) {
    return (
      <div className="rounded-xl border border-border bg-card p-8 text-center">
        ...
        <Link to="/" className="mt-4 inline-block rounded-md bg-neon px-4 py-2 font-semibold text-primary-foreground">Enter your name</Link>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-2">
          <div className="flex rounded-md border border-border bg-card p-1 text-sm">
            {(["all", "scheduled", "played"] as const).map((f) => (
              <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1 rounded ${filter === f ? "bg-neon text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}>
                {f[0].toUpperCase() + f.slice(1)}
              </button>
            ))}
          </div>
          <button onClick={() => setShowAdd(!showAdd)} className="rounded-md bg-neon px-4 py-2 font-semibold text-primary-foreground glow-neon">
            {showAdd ? "Cancel" : "+ Add Match"}
          </button>
        </div>
      </div>

      {showAdd && <AddMatchForm onDone={() => { setShowAdd(false); qc.invalidateQueries({ queryKey: ["matches"] }); }} />}
    ...
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 text-xs uppercase tracking-widest">
          ...
          {match.kickoff_at && <span className="text-muted-foreground">· {new Date(match.kickoff_at).toLocaleString()}</span>}
        </div>
        <div className="display text-2xl mt-1 truncate">
          <TeamName name={match.team_a} winner={match.result === "team_a"} /> <span className="text-muted-foreground text-lg">vs</span> <TeamName name={match.team_b} winner={match.result === "team_b"} />
          {match.result === "draw" && <span className="text-muted-foreground text-base ml-2">(Draw)</span>}
        </div>
        {match.description && <p className="text-sm text-muted-foreground mt-1 line-clamp-1">{match.description}</p>}
      </div>
      <div className="text-right shrink-0">
        {myPick ? (
          ...
          <div className="rounded-md border border-amber-400/40 bg-amber-400/10 px-3 py-1.5 text-xs text-amber-400 uppercase tracking-widest">No pick</div>
        ) : (
...
function AddMatchForm({ onDone }: { onDone: () => void }) {
  ...
  const mut = useMutation({
    ...
    onSuccess: () => { toast.success("Match added"); onDone(); },
    onError: (e: any) => toast.error(e.message),
  });

  return (
    <form
      onSubmit={(e) => { e.preventDefault(); if (!teamA.trim() || !teamB.trim()) return; mut.mutate(); }}
    ...
    >
      ...
      <div>
        <label className="block text-xs uppercase tracking-widest text-muted-foreground mb-1">Kickoff (optional)</label>
        <input type="datetime-local" value={kickoff} onChange={(e) => setKickoff(e.target.value)} className="w-full rounded-md border border-border bg-input px-3 py-2 text-sm" />
      </div>
      <div className="flex gap-2 justify-end">
        <button type="button" onClick={onDone} className="rounded-md border border-border px-4 py-2 text-sm">Cancel</button>
        <button disabled={mut.isPending} className="rounded-md bg-neon px-4 py-2 text-sm font-semibold text-primary-foreground glow-neon disabled:opacity-60">
...
function Input({ label, value, onChange, required }: { label: string; value: string; onChange: (v: string) => void; required?: boolean }) {
  return (
    <div>
      ...
      <input value={value} onChange={(e) => onChange(e.target.value)} required={required} className="w-full rounded-md border border-border bg-input px-3 py-2 text-sm" />
    </div>
  );
}
...
function Textarea({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  return (
    <div>
      <label className="block text-xs uppercase tracking-widest text-muted-foreground mb-1">{label}</label>
      <textarea value={value} onChange={(e) => onChange(e.target.value)} rows={2} className="w-full rounded-md border border-border bg-input px-3 py-2 text-sm" />
    </div>
  );
}
