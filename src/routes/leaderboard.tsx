...
function Leaderboard() {
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      ...
      const s = settingsRes.data ?? { win_points: 3, draw_points: 1, loss_points: -1, not_played_points: 0, max_not_played: 2 };
      const matchById = new Map(matches.data?.map((m) => [m.id, m]));
      const rows = new Map<string, Row>();
      players.data?.forEach((p) =>
        rows.set(p.id, { player_id: p.id, name: p.name, points: 0, wins: 0, draws: 0, losses: 0, not_played: 0, picks: 0 })
      );
      picks.data?.forEach((pk) => {
        ...
        if (m.result === "draw") { r.draws++; r.points += s.draw_points; }
        else if (m.result === pk.picked) { r.wins++; r.points += s.win_points; }
        else { r.losses++; r.points += s.loss_points; }
      });
    ...
    <div className="space-y-6">
      ...
      {q.data && q.data.length >= 1 && (
        <div className="grid sm:grid-cols-3 gap-3">
          {q.data.slice(0, 3).map((r, i) => (
            <div key={r.player_id} className={`rounded-2xl border p-5 ${i === 0 ? "border-neon bg-neon/10 glow-neon" : "border-border bg-card"}`}>
              <div className="display text-5xl text-neon">#{i + 1}</div>
              <div className="display text-2xl mt-1 truncate">{r.name}</div>
              <div className="text-xs uppercase tracking-widest text-muted-foreground mt-1">{r.points} pts · {r.wins}W {r.draws}D {r.losses}L</div>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="rounded-2xl border border-border bg-card overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {q.data?.map((r, i) => (
              <tr key={r.player_id} className={`border-t border-border ${me?.id === r.player_id ? "bg-neon/5" : ""}`}>
                <td className="px-4 py-3 text-muted-foreground">{i + 1}</td>
                <td className="px-4 py-3 font-medium">
                  {r.name}{me?.id === r.player_id && <span className="ml-2 text-xs text-neon">you</span>}
                </td>
